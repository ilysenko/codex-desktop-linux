...
help:
	...
	@printf '  %-18s %s\n' "make readiness" "Run the compact installed readiness summary"
	@printf '  %-18s %s\n' "make capability-gaps" "Run the compact Linux capability gap summary"
	@printf '  %-18s %s\n' "make parity-schema" "Check required Codex app-server schema surface"
	...
	@printf '  %-18s %s\n' "make parity-evidence" "Build one redacted browser/service/desktop/keyring evidence bundle"
...
readiness:
	@echo "[make] Running $(PACKAGE_NAME)-doctor --readiness"
	"$(or $(DOCTOR),/usr/bin/$(PACKAGE_NAME)-doctor)" --readiness

capability-gaps:
	@echo "[make] Running $(PACKAGE_NAME)-doctor --capability-gaps"
	"$(or $(DOCTOR),/usr/bin/$(PACKAGE_NAME)-doctor)" --capability-gaps

parity-schema:
	@echo "[make] Checking app-server schema parity surface"
	node scripts/app-server-schema-guard.js

parity-smoke:
	@echo "[make] Running non-sensitive desktop parity smoke"
	node scripts/desktop-parity-smoke.js

parity-browser-matrix:
	@echo "[make] Checking Linux browser integration matrix"
	node scripts/browser-matrix-smoke.js
...
parity-evidence:
	@echo "[make] Building redacted parity evidence bundle"
	node scripts/parity-evidence-bundle.js

parity-full:
	@echo "[make] Running full local desktop parity checks"
	bash scripts/desktop-parity-full.sh

parity-strict:
	@echo "[make] Running strict local desktop parity checks"
	CODEX_PARITY_STRICT=1 bash scripts/desktop-parity-full.sh
