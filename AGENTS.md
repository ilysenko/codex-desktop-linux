...
- `package-common.sh` — shared shell helpers used by the native package builders (versioning, payload staging, desktop app service rendering, user-service helper installation).
...
- Update manager:
  Default native packages include `/usr/bin/codex-update-manager`, `/usr/lib/systemd/user/codex-update-manager.service`, and a minimal rebuild bundle under `/opt/codex-desktop/update-builder`. All native packages include the opt-in app unit `/usr/lib/systemd/user/codex-desktop.service`.
- Privilege boundary:
