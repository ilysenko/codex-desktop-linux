...
| Installed setup doctor | ✅ native packages | `codex-desktop-doctor` checks package files, services, update-builder health, browser native messaging, and Computer Use readiness without reading private session contents |
...
### Installed doctor and app service
...
`make parity-smoke` starts `codex app-server --remote-control`, probes read-only app-server APIs for threads, plugin and installed-plugin summaries, apps, live session-scoped MCP status plus no-op MCP tool-call and text/blob resource-read fixtures through an ephemeral thread, skills, models, model-provider capabilities, experimental features plus an empty no-op feature enablement set, permission profiles, collaboration modes, config, managed requirements when `bwrap` can isolate `/etc/codex`, external-agent detection plus empty no-op import, filesystem metadata/directory/file reads plus isolated create/write/copy/remove/watch/unwatch filesystem behavior against temporary fixtures, a sandboxed command, and remote-control status through both an explicit status read and a status notification, then prints only sanitized counts and booleans. It also runs thread read plus goal set/get/clear, plugin detail read plus install/uninstall, and skill enable/disable config writes against separate temporary `CODEX_HOME`, `HOME`, XDG trees, repos, ephemeral threads, and synthetic local fixtures so normal user thread, plugin, and skill state is not mutated. Transient app directory HTTP errors are summarized without response bodies. It does not print thread IDs, thread names, account fields, browser state, QR data, screenshots, cookies, tokens, resource text/blob payloads, plugin IDs, plugin skill contents, skill config paths, filesystem paths, file contents, model names, feature names, permission profile IDs, collaboration mode names, or remote-control identifiers. To include optional real Electron UI presence checks, launch a separate debug-port instance yourself and pass its local CDP origin:
...
8. Native packages provide `codex-desktop-doctor` plus an opt-in `codex-desktop.service` app unit
9. Default native packages provide `codex-update-manager` plus a `systemd --user` updater service unit
