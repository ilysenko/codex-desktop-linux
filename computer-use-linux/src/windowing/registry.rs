...
pub const WINDOW_PERMISSION_HINT: &str = "Computer Use could not access a supported window list backend. Targeted window input requires session-bus access plus GNOME Shell Introspect, the Codex GNOME Shell extension, the COSMIC Wayland helper, KWin/Plasma DBus scripting, Hyprland hyprctl, Sway swaymsg, or i3-msg. On GNOME, run setup_window_targeting to install the extension backend.";
...
const DESCRIPTORS: &[BackendDescriptor] = &[
    ...
    BackendDescriptor {
        id: SWAY_BACKEND,
        failure_label: "Sway",
        list_note: "Window list came from Sway swaymsg. Terminal windows may include best-effort PTY and active-process context when the process tree is readable.",
        missing_hint: "On Sway, ensure swaymsg can reach the active Sway IPC socket.",
        can_exact_focus: true,
    },
    BackendDescriptor {
        id: I3_BACKEND,
