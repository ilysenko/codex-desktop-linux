...
const DESCRIPTORS: &[BackendDescriptor] = &[
    ...
    BackendDescriptor {
        ...
        missing_hint: "On Niri, ensure niri msg can connect to the running compositor through NIRI_SOCKET.",
        can_exact_focus: true,
    },
    BackendDescriptor {
        id: I3_BACKEND,
...
mod tests {
    ...
    #[test]
    fn advertises_niri_backend_with_exact_focus() {
        let descriptor = descriptor("niri").expect("niri backend should be registered");

        assert_eq!(descriptor.failure_label, "Niri");
        assert!(descriptor.can_exact_focus);
    }
}
