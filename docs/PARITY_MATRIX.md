...
| Workspace filesystem | Native workspace browser reads files and directories | `fs/getMetadata`, `fs/readDirectory`, and `fs/readFile` are covered against a temporary fixture with only booleans/counts recorded | `make parity-smoke`, `make parity-schema` | Add write/copy/remove fixtures only when isolated from real workspaces |
