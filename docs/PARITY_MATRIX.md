...
- [`SERVICE_LIFECYCLE_MATRIX.md`](SERVICE_LIFECYCLE_MATRIX.md) for the
  redacted read-only live systemd user-service evidence format.
...
| Thread history and goals | Native app reads local sessions and goal state | App-server thread list/read surface plus isolated thread goal set/get/clear fixture coverage are present | `make parity-smoke`, `make parity-schema` | Avoid printing thread ids, names, goals, or content in logs |
| Workspace filesystem | Native workspace browser reads files and directories | `fs/getMetadata`, `fs/readDirectory`, `fs/readFile`, `fs/createDirectory`, `fs/writeFile`, `fs/copy`, `fs/remove`, `fs/watch`, and `fs/unwatch` are covered against temporary fixtures with only booleans/counts recorded | `make parity-smoke`, `make parity-schema` | Add deeper file-tree UX checks only when they stay isolated and redacted |
| Plugins and marketplace | Built-in app support | Plugin list, installed-plugin summary, bundled marketplace cache, and isolated plugin detail read plus install/uninstall coverage against a temporary `CODEX_HOME` and synthetic local marketplace | `make parity-smoke`, `make doctor` | Keep mutating plugin checks isolated from real user state |
...
| Skills | Built-in app support | Skills list API plus repo-scoped fixture discovery and isolated enable/disable config-write coverage present | `make parity-smoke`, `make parity-schema` | Keep mutating skill checks isolated from real user config |
