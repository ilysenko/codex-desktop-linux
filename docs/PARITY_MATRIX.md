...
| Workspace filesystem | Native workspace browser reads files and directories | `fs/getMetadata`, `fs/readDirectory`, `fs/readFile`, `fs/createDirectory`, `fs/writeFile`, `fs/copy`, and `fs/remove` are covered against temporary fixtures with only booleans/counts recorded | `make parity-smoke`, `make parity-schema` | Add watch/unwatch only if it stays isolated and redacted |
