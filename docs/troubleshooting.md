...
| Stale install / cached package | `make build-app-fresh` refreshes the cached package and builds a clean candidate; the working app remains until acceptance succeeds |
| `install.sh failed during local rebuild` after an upstream package update, with `Critical patch failures` or a `required-patch` blocker | The installed Linux wrapper may be older than the package. Check `~/.local/state/codex-update-manager/service.log` for the workspace `install.log` path and inspect `/opt/codex-desktop/update-builder/.codex-linux/source-info.json`. Update the wrapper/package from a current trusted checkout or package source, then retry. Do not use `CODEX_ENFORCE_CRITICAL_PATCHES=0` for routine updates; if current `main` still rejects the package, report the drift with the acceptance decision and patch report. |
| `Candidate was not installed (verdict: rejected)` | Open `dist-next/rebuild/upstream-dmg-decision.json`. If the blocker is `enabled-feature-drift`, disable the named Linux Feature and retry; otherwise fix required current-package drift before retrying |
...
from the same package or a fresh one:

```bash
git pull --ff-only
make build-app ARTIFACT=~/.cache/codex-update-manager/downloads/ChatGPT.deb
make package
make install
```
