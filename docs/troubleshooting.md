...
| Chrome side panel reports `Codex app-server manifest entry is missing required path parent` | Do not edit either native-host manifest. The native host rejects a runtime path whose ancestor is symlinked, owned by another non-root user, or group/world writable. Inspect the packaged path with `namei -l /opt/codex-desktop/resources` and any extended ACL with `getfacl -cp /opt`; make only the intended package ancestors non-writable (for a standard install, `/opt` is normally `0755`), then retry the panel or fully restart Chrome and ChatGPT Desktop. If the shared writable ancestor is intentional, reinstall under a trusted non-writable root instead of weakening the validation. |
