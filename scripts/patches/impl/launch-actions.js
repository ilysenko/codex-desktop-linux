...
function applyLinuxSettingsPersistencePatch(currentSource) {
  ...
  if (!patchedSource.includes("function codexLinuxPersistSettingsState(")) {
    ...
    const legacySettingsPathRegex =
      /function codexLinuxSettingsPath\(\)\{let ([A-Za-z_$][\w$]*)=process\.env\.XDG_CONFIG_HOME\|\|process\.env\.HOME&&([A-Za-z_$][\w$]*)\.join\(process\.env\.HOME,`\.config`\);return \1\?\2\.join\(\1,`(?:codex-desktop|chatgpt-desktop)`,`settings\.json`\):null\}/;
    patchedSource = patchedSource.replace(
      legacySettingsPathRegex,
      (_match, _configVar, pathVar) =>
        `function codexLinuxSettingsAppId(){let e=process.env.CODEX_LINUX_APP_ID||process.env.CODEX_APP_ID||\`chatgpt-desktop\`;return/^[A-Za-z0-9._-]+$/.test(e)?e:\`chatgpt-desktop\`}function codexLinuxSettingsPath(){let e=process.env.CODEX_LINUX_SETTINGS_FILE;if(typeof e===\`string\`&&e.length>0)return e;let t=process.env.XDG_CONFIG_HOME||process.env.HOME&&${pathVar}.join(process.env.HOME,\`.config\`);return t?${pathVar}.join(t,codexLinuxSettingsAppId(),\`settings.json\`):null}`,
    );
  }
