...
function settingsPersistenceHelpersSource(pathVar, fsVar, stateFileVar) {
  return [
    ...
    `async function codexLinuxOpenRegularSettingsLock(codexLinuxLockPath,codexLinuxFlags){let codexLinuxLockFile=await ${fsVar}.promises.open(codexLinuxLockPath,codexLinuxFlags|${fsVar}.constants.O_NOFOLLOW);try{let codexLinuxLockStat=await codexLinuxLockFile.stat({bigint:!0});if(!codexLinuxLockStat.isFile()){let codexLinuxLockTypeError=Error(\`Settings lock is not a regular file: ${"${codexLinuxLockPath}"}\`);throw codexLinuxLockTypeError.code=\`EINVAL\`,codexLinuxLockTypeError}return{file:codexLinuxLockFile,stat:codexLinuxLockStat}}catch(codexLinuxLockOpenError){throw await codexLinuxLockFile.close(),codexLinuxLockOpenError}}`,
    `function codexLinuxSameSettingsLock(codexLinuxLeftStat,codexLinuxRightStat){return codexLinuxLeftStat.dev===codexLinuxRightStat.dev&&codexLinuxLeftStat.ino===codexLinuxRightStat.ino}`,
    `async function codexLinuxReleaseSettingsLock(codexLinuxLock){let codexLinuxOpenedLock=null;try{codexLinuxOpenedLock=await codexLinuxOpenRegularSettingsLock(codexLinuxLock.path,${fsVar}.constants.O_RDONLY),(await codexLinuxOpenedLock.file.readFile(\`utf8\`)).split(\`\\n\`,1)[0]===codexLinuxLock.owner&&await ${fsVar}.promises.unlink(codexLinuxLock.path)}catch{}finally{codexLinuxOpenedLock!=null&&await codexLinuxOpenedLock.file.close()}}`,
    ...
    `function codexLinuxLiveRecoveryClaimOwner(codexLinuxRecord,codexLinuxNow){if(!codexLinuxRecord.startsWith(\`recover:\`))return null;let codexLinuxClaim=codexLinuxRecord.slice(8),codexLinuxSeparator=codexLinuxClaim.lastIndexOf(\`:\`);if(codexLinuxSeparator<0)return null;let codexLinuxOwner=codexLinuxClaim.slice(0,codexLinuxSeparator),codexLinuxExpires=Number(codexLinuxClaim.slice(codexLinuxSeparator+1));return Number.isFinite(codexLinuxExpires)&&codexLinuxExpires>codexLinuxNow&&codexLinuxSettingsOwnerIsAlive(codexLinuxOwner)?codexLinuxOwner:null}`,
    `async function codexLinuxRecoverSettingsLock(codexLinuxLockPath,codexLinuxClaimOwner){let codexLinuxRecoveryFlags=${fsVar}.constants.O_RDWR|(${fsVar}.constants.O_NOFOLLOW==null?0:${fsVar}.constants.O_APPEND),codexLinuxOpenedLock=await codexLinuxOpenRegularSettingsLock(codexLinuxLockPath,codexLinuxRecoveryFlags);try{let codexLinuxLockSnapshot=await codexLinuxOpenedLock.file.readFile(\`utf8\`),codexLinuxOriginalOwner=codexLinuxLockSnapshot.split(\`\\n\`,1)[0],codexLinuxNow=Date.now();if(codexLinuxNow-Number(codexLinuxOpenedLock.stat.mtimeMs)<=3e4||codexLinuxSettingsOwnerIsAlive(codexLinuxOriginalOwner))return!1;let codexLinuxClaim=\`recover:${"${codexLinuxClaimOwner}"}:${"${codexLinuxNow+1e3}"}\`;await codexLinuxOpenedLock.file.write((codexLinuxLockSnapshot.endsWith(\`\\n\`)?\`\`:\`\\n\`)+codexLinuxClaim+\`\\n\`),await codexLinuxOpenedLock.file.sync();let codexLinuxCurrentLock=await codexLinuxOpenRegularSettingsLock(codexLinuxLockPath,${fsVar}.constants.O_RDONLY);try{if(!codexLinuxSameSettingsLock(codexLinuxOpenedLock.stat,codexLinuxCurrentLock.stat))return!1;codexLinuxLockSnapshot=await codexLinuxCurrentLock.file.readFile(\`utf8\`)}finally{await codexLinuxCurrentLock.file.close()}if(!codexLinuxLockSnapshot.endsWith(\`\\n\`)||codexLinuxLockSnapshot.split(\`\\n\`,1)[0]!==codexLinuxOriginalOwner)return!1;let codexLinuxFirstLiveClaim=codexLinuxLockSnapshot.split(\`\\n\`).slice(1,-1).map(codexLinuxRecord=>codexLinuxLiveRecoveryClaimOwner(codexLinuxRecord,codexLinuxNow)).find(Boolean);if(codexLinuxFirstLiveClaim!==codexLinuxClaimOwner)return!1;let codexLinuxTakeoverLock=await codexLinuxOpenRegularSettingsLock(codexLinuxLockPath,${fsVar}.constants.O_RDWR);try{if(!codexLinuxSameSettingsLock(codexLinuxOpenedLock.stat,codexLinuxTakeoverLock.stat))return!1;let codexLinuxTakeoverWrite=await codexLinuxTakeoverLock.file.write(codexLinuxClaimOwner+\`\\n\`,0,\`utf8\`);await codexLinuxTakeoverLock.file.truncate(codexLinuxTakeoverWrite.bytesWritten),await codexLinuxTakeoverLock.file.sync()}finally{await codexLinuxTakeoverLock.file.close()}let codexLinuxVerifiedLock=await codexLinuxOpenRegularSettingsLock(codexLinuxLockPath,${fsVar}.constants.O_RDONLY);try{return codexLinuxSameSettingsLock(codexLinuxOpenedLock.stat,codexLinuxVerifiedLock.stat)&&(await codexLinuxVerifiedLock.file.readFile(\`utf8\`)).split(\`\\n\`,1)[0]===codexLinuxClaimOwner}finally{await codexLinuxVerifiedLock.file.close()}}finally{await codexLinuxOpenedLock.file.close()}}`,
    `async function codexLinuxAcquireSettingsLock(codexLinuxSettingsPathValue){let codexLinuxLockPath=\`${"${codexLinuxSettingsPathValue}"}.lock\`,codexLinuxLockOwner=codexLinuxSettingsOwner(),codexLinuxLockDeadline=Date.now()+2e3;for(;;)try{let codexLinuxLockFile=await ${fsVar}.promises.open(codexLinuxLockPath,\`wx\`,384),codexLinuxLockInitError=null;try{await codexLinuxLockFile.writeFile(codexLinuxLockOwner+\`\\n\`),await codexLinuxLockFile.sync()}catch(codexLinuxLockWriteError){codexLinuxLockInitError=codexLinuxLockWriteError}finally{await codexLinuxLockFile.close()}if(codexLinuxLockInitError!=null){try{await ${fsVar}.promises.unlink(codexLinuxLockPath)}catch{}throw codexLinuxLockInitError}return{path:codexLinuxLockPath,owner:codexLinuxLockOwner}}catch(codexLinuxLockError){if(codexLinuxLockError?.code!==\`EEXIST\`)throw codexLinuxLockError;try{if(await codexLinuxRecoverSettingsLock(codexLinuxLockPath,codexLinuxLockOwner))return{path:codexLinuxLockPath,owner:codexLinuxLockOwner}}catch(codexLinuxRecoveryError){if(codexLinuxRecoveryError?.code!==\`ENOENT\`)throw codexLinuxRecoveryError}if(Date.now()>=codexLinuxLockDeadline)throw Error(\`Timed out waiting for settings lock ${"${codexLinuxLockPath}"}\`);await new Promise(codexLinuxRetry=>setTimeout(codexLinuxRetry,25))}}`,
    `async function codexLinuxPersistSettingsState(codexLinuxSettingsKey,codexLinuxSettingsNewValue){if(process.platform!==\`linux\`||!${persistedLinuxSettingsKeysSource()}.includes(codexLinuxSettingsKey))return;let codexLinuxSettingsLock=null,codexLinuxSettingsTempPath=null;try{let codexLinuxSettingsPathValue=codexLinuxSettingsPath();if(!codexLinuxSettingsPathValue)return;await ${fsVar}.promises.mkdir(${pathVar}.dirname(codexLinuxSettingsPathValue),{recursive:!0,mode:448}),codexLinuxSettingsLock=await codexLinuxAcquireSettingsLock(codexLinuxSettingsPathValue);let codexLinuxSettingsObject=codexLinuxReadSettingsFile();codexLinuxSettingsNewValue===void 0?delete codexLinuxSettingsObject[codexLinuxSettingsKey]:codexLinuxSettingsObject[codexLinuxSettingsKey]=codexLinuxSettingsNewValue,codexLinuxSettingsTempPath=\`${"${codexLinuxSettingsPathValue}"}.tmp.${"${codexLinuxSettingsOwner()}"}\`;let codexLinuxSettingsTempFile=await ${fsVar}.promises.open(codexLinuxSettingsTempPath,\`wx\`,384);try{await codexLinuxSettingsTempFile.writeFile(JSON.stringify(codexLinuxSettingsObject,null,2)+\`\\n\`,\`utf8\`),await codexLinuxSettingsTempFile.sync()}finally{await codexLinuxSettingsTempFile.close()}await ${fsVar}.promises.rename(codexLinuxSettingsTempPath,codexLinuxSettingsPathValue),codexLinuxSettingsTempPath=null;if(${fsVar}.constants.O_DIRECTORY!=null){let codexLinuxSettingsDirectory=await ${fsVar}.promises.open(${pathVar}.dirname(codexLinuxSettingsPathValue),${fsVar}.constants.O_RDONLY|${fsVar}.constants.O_DIRECTORY);try{await codexLinuxSettingsDirectory.sync()}finally{await codexLinuxSettingsDirectory.close()}}}catch(codexLinuxSettingsWriteError){}finally{if(codexLinuxSettingsTempPath!=null)try{await ${fsVar}.promises.unlink(codexLinuxSettingsTempPath)}catch{}codexLinuxSettingsLock!=null&&await codexLinuxReleaseSettingsLock(codexLinuxSettingsLock)}}`,
  ].join("");
}
