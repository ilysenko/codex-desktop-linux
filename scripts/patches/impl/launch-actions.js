...
function applyLinuxSettingsPersistencePatch(currentSource) {
  ...
  if (!patchedSource.includes("function codexLinuxPersistSettingsState(")) {
    // Resolve these modules inside the generated helpers. A bundle-wide alias
    // lookup can accidentally capture a minified name from an unrelated local
    // scope, leaving the persisted Linux settings unreadable at runtime.
    const stateFileHelperSource =
      (stateFileVar) =>
        `${stateFileVar == null ? "" : `var ${stateFileVar}=\`.codex-global-state.json\`;`}function codexLinuxSettingsAppId(){let e=process.env.CODEX_LINUX_APP_ID||process.env.CODEX_APP_ID||\`codex-desktop\`;return/^[A-Za-z0-9._-]+$/.test(e)?e:\`codex-desktop\`}function codexLinuxSettingsPath(){let __codexLinuxPath=require(\`node:path\`),e=process.env.CODEX_LINUX_SETTINGS_FILE;if(typeof e===\`string\`&&e.length>0)return e;let t=process.env.XDG_CONFIG_HOME||process.env.HOME&&__codexLinuxPath.join(process.env.HOME,\`.config\`);return t?__codexLinuxPath.join(t,codexLinuxSettingsAppId(),\`settings.json\`):null}function codexLinuxReadSettingsFile(){let __codexLinuxFs=require(\`node:fs\`),e=codexLinuxSettingsPath();if(!e||!__codexLinuxFs.existsSync(e))return{};try{let t=__codexLinuxFs.readFileSync(e,\`utf8\`),n=JSON.parse(t);return n&&typeof n===\`object\`&&!Array.isArray(n)?n:{}}catch(e){return{}}}function codexLinuxPersistSettingsState(e,t){if(process.platform!==\`linux\`||!${persistedLinuxSettingsKeysSource()}.includes(e))return;try{let __codexLinuxFs=require(\`node:fs\`),__codexLinuxPath=require(\`node:path\`),n=codexLinuxSettingsPath();if(!n)return;let r=codexLinuxReadSettingsFile();t===void 0?delete r[e]:r[e]=t,__codexLinuxFs.mkdirSync(__codexLinuxPath.dirname(n),{recursive:!0,mode:448}),__codexLinuxFs.writeFileSync(n,JSON.stringify(r,null,2)+\`\\n\`,\`utf8\`)}catch(e){}}`;
