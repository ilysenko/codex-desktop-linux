...
function applyLinuxTrayPatch(currentSource, iconPathExpression) {
  ...
  const packagedTrayIconPathExpression =
    "process.resourcesPath+`/../.codex-linux/`+(process.env.CODEX_LINUX_APP_ID??`chatgpt-desktop`)+`-tray.png`";
  const packagedAppIconPathExpression =
    "process.resourcesPath+`/../.codex-linux/`+(process.env.CODEX_LINUX_APP_ID??`chatgpt-desktop`)+`.png`";
  ...
  if (iconPathExpression != null) {
    ...
    const splitTrayIconFallbackRegex =
      /let ([A-Za-z_$][\w$]*)=([A-Za-z_$][\w$]*)\(([A-Za-z_$][\w$]*),([A-Za-z_$][\w$]*),([A-Za-z_$][\w$]*)\);return \1==null\?\{defaultIcon:await ([A-Za-z_$][\w$]*)\.app\.getFileIcon\(process\.execPath,\{size:((?:process\.platform===`win32`\?`small`:`normal`)|`small`|`normal`)\}\),chronicleRunningIcon:null\}:\{defaultIcon:\1,chronicleRunningIcon:null\}/;
    if (
    ...
    } else if (splitTrayIconFallbackRegex.test(patchedSource)) {
      patchedSource = patchedSource.replace(
        splitTrayIconFallbackRegex,
        (_match, iconVar, resolverVar, buildFlavorVar, appBrandVar, repoRootVar, electronAlias, sizeExpression) =>
          `let ${iconVar}=${resolverVar}(${buildFlavorVar},${appBrandVar},${repoRootVar});if(${iconVar}==null&&process.platform===\`linux\`){let __codexLinuxTrayIcon=${electronAlias}.nativeImage.createFromPath(${packagedTrayIconPathExpression});if(!__codexLinuxTrayIcon.isEmpty())return{defaultIcon:__codexLinuxTrayIcon,chronicleRunningIcon:null};let __codexLinuxAppIcon=${electronAlias}.nativeImage.createFromPath(${packagedAppIconPathExpression});if(!__codexLinuxAppIcon.isEmpty())return{defaultIcon:__codexLinuxAppIcon,chronicleRunningIcon:null};let __codexLinuxUpstreamTrayIcon=${electronAlias}.nativeImage.createFromPath(${iconPathExpression});if(!__codexLinuxUpstreamTrayIcon.isEmpty())return{defaultIcon:__codexLinuxUpstreamTrayIcon,chronicleRunningIcon:null}}return ${iconVar}==null?{defaultIcon:await ${electronAlias}.app.getFileIcon(process.execPath,{size:${sizeExpression}}),chronicleRunningIcon:null}:{defaultIcon:${iconVar},chronicleRunningIcon:null}`,
      );
    } else {
