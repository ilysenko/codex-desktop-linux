...
function buildLinuxExternalOpenHelpers() {
  return (
    ...
    `let __codexLdState=process.env.CODEX_LINUX_ORIGINAL_LD_LIBRARY_PATH_STATE,__codexLdValue=process.env.CODEX_LINUX_ORIGINAL_LD_LIBRARY_PATH_VALUE;__codexLdState===\`unset\`?delete __codexEnv.LD_LIBRARY_PATH:__codexLdState===\`empty\`?__codexEnv.LD_LIBRARY_PATH=\`\`:__codexLdState===\`value\`&&typeof __codexLdValue===\`string\`?__codexEnv.LD_LIBRARY_PATH=__codexLdValue:delete __codexEnv.LD_LIBRARY_PATH;` +
    `for(let __codexKey of[\`CODEX_LINUX_ORIGINAL_LD_LIBRARY_PATH_STATE\`,\`CODEX_LINUX_ORIGINAL_LD_LIBRARY_PATH_VALUE\`,\`CODEX_LINUX_APP_LD_LIBRARY_PATH\`])delete __codexEnv[__codexKey];` +
    `return __codexEnv}` +
