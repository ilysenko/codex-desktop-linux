...
function quitFunctionRange(source) {
  ...
  const starts = new Set(markers.map((match) => source.lastIndexOf("function ", match.index)));
  if (markers.length < 1 || markers.length > 2 || starts.size !== 1) {
    throw new Error("Expected one official Quit confirmation");
  }
  const start = starts.values().next().value;
  const endMarker = source.indexOf("}function ", markers[0].index);
  if (start < 0 || endMarker < 0) throw new Error("Could not isolate official Quit handler");
...
function applyQuitConfirmationFocus(source) {
  ...
  if (helperPresent) {
    if (body.includes("codexLinuxQuitPromptPending") &&
        !body.includes("showMessageBoxSync(")) return source;
    throw new Error("Incomplete Quit confirmation focus patch");
  }

  const headerEnd = body.indexOf("}){");
  if (headerEnd < 0) throw new Error("Official Quit handler signature changed");
  const header = body.slice(0, headerEnd + 3);
  const binding = (name) => {
    const matches = [...header.matchAll(new RegExp(`${name}:(${IDENT})(?=,|\\})`, "g"))];
    if (matches.length !== 1) throw new Error(`Official Quit binding changed: ${name}`);
    return matches[0][1];
  };
  const windows = binding("windows");
  const quitState = binding("quitState");
  const beforeQuit = [...body.matchAll(/([A-Za-z_$][\w$]*)\.app\.on\(`before-quit`,([A-Za-z_$][\w$]*)=>\{/g)];
  if (beforeQuit.length !== 1) throw new Error("Expected one official before-quit handler");
  const [, appAlias, eventName] = beforeQuit[0];
  const confirmation = new RegExp(
    `if\\(${appAlias}\\.dialog\\.showMessageBoxSync\\((\\{[\\s\\S]*?\\})\\)!==0\\)\\{${eventName}\\.preventDefault\\(\\);return\\}` +
  ...
  let patchedBody = body.replace(confirmation, (_match, options) =>
    ...
    `Promise.resolve().then(()=>codexLinuxParent?${appAlias}.dialog.showMessageBox(codexLinuxParent,${options}):` +
    `${appAlias}.dialog.showMessageBox(${options})).then(e=>{codexLinuxQuitPromptPending=!1;` +
    `if(e.response===0){${quitState}.markQuitApproved();${appAlias}.app.quit()}}).catch(e=>{` +
    "codexLinuxQuitPromptPending=!1;console.error(`[codex-desktop] Quit confirmation failed`,e)});return");
  patchedBody = patchedBody.replace(beforeQuit[0][0],
    `${beforeQuit[0][0]}if(codexLinuxQuitPromptPending){${eventName}.preventDefault();return}`);
...
module.exports = { applyQuitConfirmationFocus, descriptors, HELPER_SOURCE };
