...
const SHOW_HELPER_NAME = "codexLinuxShowQuitDialog";
const HELPER_SOURCE =
  ...
  `function ${SHOW_HELPER_NAME}(e,t,n){return t?e.showMessageBox(t,n):e.showMessageBox(n)}`;
...
function quitFunctionRange(source) {
  ...
  if (markers.length !== 1) {
    throw new Error(`Expected exactly one official Quit confirmation, found ${markers.length}`);
  }
...
function applyQuitConfirmationFocus(source) {
  ...
  let patchedBody = body.replace(confirmation, (_match, options) =>
    ...
    `Promise.resolve().then(()=>${SHOW_HELPER_NAME}(${appAlias}.dialog,codexLinuxParent,${options})).then(e=>{` +
