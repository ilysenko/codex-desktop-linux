...
function linuxBuildInfoPanelSource() {
  return `function codexLinuxBuildInfoValue(value,fallback="unknown"){return typeof value=="string"&&value.trim().length>0?value:Array.isArray(value)&&value.length>0?value.join(", "):value==null?fallback:String(value)}function codexLinuxBuildInfoRows(payload){let info=payload?.info;if(!info)return [["Metadata file",codexLinuxBuildInfoValue(payload?.path,"not found")]];let target=info.linuxTarget??{},distro=target.distro??{},dmg=info.upstreamDmg??{},source=info.source??{},features=info.linuxFeatures?.enabled??[],profile=info.packageProfile??{},workspace=info.workspace??{},rootCertificate=workspace.rootCertificate??{},workflowHarness=workspace.workflowHarness??{},repoPath=source.repoPath??null,gitStatus=source.dirty===null?"unknown":source.dirty?"dirty":"clean",commit=source.commit||source.shortCommit||"",commitValue=commit?source.dirty?\`\${commit} (dirty)\`:commit:"unknown",distroValue=distro.prettyName||[distro.id,distro.versionId].filter(Boolean).join(" ")||"unknown",rootCertificateValue=codexLinuxBuildInfoValue(rootCertificate.status,rootCertificate.path?"present":"missing"),workflowHarnessValue=codexLinuxBuildInfoValue(workflowHarness.status,workflowHarness.path?"available":"missing");return [["Metadata file",codexLinuxBuildInfoValue(payload?.path)],["Linux package profile",codexLinuxBuildInfoValue(profile.label)],["Distro",distroValue],["Package manager",codexLinuxBuildInfoValue(target.packageManager??profile.packageManager)],["Package format",codexLinuxBuildInfoValue(target.packageFormat??profile.format)],["Repo location",codexLinuxBuildInfoValue(repoPath)],["Git remote",codexLinuxBuildInfoValue(source.remote,null)],["Git branch",codexLinuxBuildInfoValue(source.branch)],["Git status",gitStatus],["Root certificate",rootCertificateValue],["Workflow harness",workflowHarnessValue],["Enabled features",features.length>0?features.join(", "):"none"],["Upstream app version",codexLinuxBuildInfoValue(dmg.appVersion)],["Upstream DMG SHA256",codexLinuxBuildInfoValue(dmg.sha256)],["Electron",codexLinuxBuildInfoValue(info.electronVersion)],["Linux source commit",commitValue],["Source commit URL",codexLinuxBuildInfoValue(payload?.commitUrl,null)],["Source branch",codexLinuxBuildInfoValue(source.branch)],["Generated",codexLinuxBuildInfoValue(info.generatedAt)]].filter(row=>row[1]!=null)}function codexLinuxBuildInfoSummary(payload){let info=payload?.info;if(!info)return [["Metadata file",codexLinuxBuildInfoValue(payload?.path,"not found")]];let target=info.linuxTarget??{},distro=target.distro??{},source=info.source??{},features=info.linuxFeatures?.enabled??[],profile=info.packageProfile??{},workspace=info.workspace??{},rootCertificate=workspace.rootCertificate??{},workflowHarness=workspace.workflowHarness??{},repoPath=source.repoPath??null,gitStatus=source.dirty===null?"unknown":source.dirty?"dirty":"clean",commit=source.commit||source.shortCommit||"",commitValue=commit?source.dirty?\`\${commit} (dirty)\`:commit:"unknown",distroValue=distro.prettyName||[distro.id,distro.versionId].filter(Boolean).join(" ")||"unknown",rootCertificateValue=codexLinuxBuildInfoValue(rootCertificate.status,rootCertificate.path?"present":"missing"),workflowHarnessValue=codexLinuxBuildInfoValue(workflowHarness.status,workflowHarness.path?"available":"missing");return [["Linux package profile",codexLinuxBuildInfoValue(profile.label)],["Distro",distroValue],["Repo location",codexLinuxBuildInfoValue(repoPath)],["Git remote",codexLinuxBuildInfoValue(source.remote,null)],["Git branch",codexLinuxBuildInfoValue(source.branch)],["Git status",gitStatus],["Root certificate",rootCertificateValue],["Workflow harness",workflowHarnessValue],["Enabled features",features.length>0?features.join(", "):"none"],["Linux source commit",commitValue],["Source branch",codexLinuxBuildInfoValue(source.branch)],["Generated",codexLinuxBuildInfoValue(info.generatedAt)]].filter(row=>row[1]!=null)}function LinuxBuildInfoPanel(){let[data,setData]=React.useState(null),[isLoading,setIsLoading]=React.useState(!0),[error,setError]=React.useState(null),[copied,setCopied]=React.useState(!1),[repoCopied,setRepoCopied]=React.useState(!1),load=React.useCallback(()=>{setIsLoading(!0),setError(null),__post("codex-linux-get-build-info",{}).then(result=>setData(result)).catch(err=>setError(err instanceof Error?err.message:String(err))).finally(()=>setIsLoading(!1))},[]);React.useEffect(()=>{load()},[load]);let info=data?.info,repoPath=info?.source?.repoPath||"",commit=info?.source?.commit||"",commitUrl=data?.commitUrl||"",copyRepo=()=>{if(!repoPath)return;let fail=err=>setError(err instanceof Error?err.message:String(err));navigator.clipboard?.writeText?navigator.clipboard.writeText(repoPath).then(()=>{setRepoCopied(!0),setTimeout(()=>setRepoCopied(!1),1500)}).catch(fail):fail("Clipboard API is unavailable")},copyCommit=()=>{if(!commit)return;let fail=err=>setError(err instanceof Error?err.message:String(err));navigator.clipboard?.writeText?navigator.clipboard.writeText(commit).then(()=>{setCopied(!0),setTimeout(()=>setCopied(!1),1500)}).catch(fail):fail("Clipboard API is unavailable")},openCommit=()=>{commitUrl&&__post("codex-linux-open-build-info-commit",{}).catch(err=>setError(err instanceof Error?err.message:String(err)))},showDetails=()=>__post("codex-linux-show-build-info",{}).catch(err=>setError(err instanceof Error?err.message:String(err))),buttonClass="h-8 rounded-md border border-token-border-default px-3 text-sm text-token-text-primary hover:bg-token-surface-secondary disabled:opacity-60",rows=codexLinuxBuildInfoSummary(data),description=isLoading?$.jsx("span",{children:"Loading build metadata..."}):$.jsxs("div",{className:"flex flex-col gap-1 text-sm",children:[...rows.map(([label,value])=>$.jsxs("div",{className:"flex items-center justify-between gap-2",children:[$.jsx("span",{className:"text-token-text-tertiary",children:label}),$.jsx("code",{className:"select-text break-all rounded bg-token-bg-secondary px-1 py-0.5 font-mono text-xs text-token-text-primary",children:value})]},label)),error?$.jsx("span",{className:"text-token-error-foreground",children:error}):null,repoCopied?$.jsx("span",{className:"text-token-text-secondary",children:"Repo path copied"}):null,copied?$.jsx("span",{className:"text-token-text-secondary",children:"Commit copied"}):null]});return $.jsxs("div",{className:"flex flex-col gap-3 rounded-xl border border-token-border-default bg-token-bg-secondary/50 p-4",children:[$.jsxs("div",{className:"flex flex-col gap-1",children:[$.jsx("div",{className:"text-sm font-medium text-token-text-primary",children:"Workspace status"}),$.jsx("div",{className:"text-sm text-token-text-secondary",children:"Use this to confirm the active repo, branch, and root-certificate context before changing anything."})]}),$.jsxs("div",{className:"grid gap-2",children:[...rows.map(([label,value])=>$.jsxs("div",{className:"flex items-center justify-between gap-2 rounded-lg border border-token-border-default bg-token-bg-primary px-3 py-2",children:[$.jsx("span",{className:"text-token-text-tertiary",children:label}),$.jsx("code",{className:"select-text break-all rounded bg-token-bg-secondary px-1 py-0.5 font-mono text-xs text-token-text-primary",children:value})]},label)),error?$.jsx("div",{className:"text-sm text-token-error-foreground",children:error}):null,repoCopied?$.jsx("div",{className:"text-sm text-token-text-secondary",children:"Repo path copied"}):null,copied?$.jsx("div",{className:"text-sm text-token-text-secondary",children:"Commit copied"}):null]}),$.jsxs("div",{className:"flex flex-wrap items-center gap-2",children:[$.jsx("button",{type:"button",className:buttonClass,disabled:!repoPath,onClick:copyRepo,children:"Copy repo path"}),$.jsx("button",{type:"button",className:buttonClass,disabled:!commit,onClick:copyCommit,children:"Copy commit"}),$.jsx("button",{type:"button",className:buttonClass,disabled:!commitUrl,onClick:openCommit,children:"Open commit"}),$.jsx("button",{type:"button",className:buttonClass,disabled:isLoading,onClick:load,children:"Refresh"}),$.jsx("button",{type:"button",className:buttonClass,onClick:showDetails,children:"Details"})]})]})}`;
}
