...
const { buildMainBuildInfoI18nSource } = require("../linux-build-info-i18n.js");
...
function buildLinuxBuildInfoHelpers(electronVar, fsVar, pathVar) {
  return `${buildMainBuildInfoI18nSource(`${electronVar}.app?.getLocale?.()`)}function codexLinuxBuildInfoPaths(){let __codexBuildInfoPaths=[];try{__codexBuildInfoPaths.push((0,${pathVar}.join)(process.resourcesPath,"codex-linux-build-info.json")),__codexBuildInfoPaths.push((0,${pathVar}.join)(process.resourcesPath,"..",".codex-linux","build-info.json"))}catch{}return __codexBuildInfoPaths}function codexLinuxReadBuildInfo(){for(let __codexBuildInfoPath of codexLinuxBuildInfoPaths())try{if(${fsVar}.existsSync(__codexBuildInfoPath)){let __codexBuildInfo=JSON.parse(${fsVar}.readFileSync(__codexBuildInfoPath,"utf8"));if(__codexBuildInfo&&typeof __codexBuildInfo==="object"&&!Array.isArray(__codexBuildInfo))return{info:__codexBuildInfo,path:__codexBuildInfoPath}}}catch{}return{info:null,path:null}}function codexLinuxBuildInfoValue(__codexBuildInfoValue,__codexBuildInfoFallback="unknown"){return typeof __codexBuildInfoValue==="string"&&__codexBuildInfoValue.trim().length>0?__codexBuildInfoValue:Array.isArray(__codexBuildInfoValue)&&__codexBuildInfoValue.length>0?__codexBuildInfoValue.join(", "):__codexBuildInfoValue==null?__codexBuildInfoFallback:String(__codexBuildInfoValue)}function codexLinuxBuildInfoCommitUrl(__codexBuildInfo){let __codexBuildInfoCommitUrl=__codexBuildInfo?.source?.commitUrl;return typeof __codexBuildInfoCommitUrl==="string"&&/^https:\\/\\/github\\.com\\/[^/\\s]+\\/[^/\\s]+\\/commit\\/[0-9a-f]{7,40}$/i.test(__codexBuildInfoCommitUrl)?__codexBuildInfoCommitUrl:null}function codexLinuxGetBuildInfo(){let __codexBuildInfoResult=codexLinuxReadBuildInfo();return{...__codexBuildInfoResult,commitUrl:codexLinuxBuildInfoCommitUrl(__codexBuildInfoResult.info)}}function codexLinuxBuildInfoDetail(__codexBuildInfo,__codexBuildInfoPath){let{locale:__codexBuildInfoLocale,t:__codexBuildInfoText}=codexLinuxBuildInfoI18n();if(!__codexBuildInfo)return __codexBuildInfoText("noMetadataFileFound");let __codexBuildInfoTarget=__codexBuildInfo.linuxTarget??{},__codexBuildInfoDistro=__codexBuildInfoTarget.distro??{},__codexBuildInfoDmg=__codexBuildInfo.upstreamDmg??{},__codexBuildInfoSource=__codexBuildInfo.source??{},__codexBuildInfoFeatures=__codexBuildInfo.linuxFeatures?.enabled??[],__codexBuildInfoProfile=__codexBuildInfo.packageProfile??{},__codexBuildInfoCommit=__codexBuildInfoSource.commit||__codexBuildInfoSource.shortCommit,__codexBuildInfoCommitValue=__codexBuildInfoCommit?__codexBuildInfoSource.dirty?__codexBuildInfoCommit+__codexBuildInfoText("dirtySuffix"):__codexBuildInfoCommit:__codexBuildInfoText("unknown"),__codexBuildInfoDistroValue=__codexBuildInfoDistro.prettyName||[__codexBuildInfoDistro.id,__codexBuildInfoDistro.versionId].filter(Boolean).join(" ")||__codexBuildInfoText("unknown"),__codexBuildInfoCommitLink=codexLinuxBuildInfoCommitUrl(__codexBuildInfo),__codexBuildInfoGenerated=codexLinuxFormatBuildInfoTimestamp(__codexBuildInfo.generatedAt,__codexBuildInfoLocale,__codexBuildInfoText("unknown"));return[__codexBuildInfoText("metadataFile")+": "+codexLinuxBuildInfoValue(__codexBuildInfoPath,__codexBuildInfoText("notFound")),__codexBuildInfoText("linuxPackageProfile")+": "+codexLinuxBuildInfoValue(__codexBuildInfoProfile.label,__codexBuildInfoText("unknown")),__codexBuildInfoText("distro")+": "+__codexBuildInfoDistroValue,__codexBuildInfoText("packageManager")+": "+codexLinuxBuildInfoValue(__codexBuildInfoTarget.packageManager??__codexBuildInfoProfile.packageManager,__codexBuildInfoText("unknown")),__codexBuildInfoText("packageFormat")+": "+codexLinuxBuildInfoValue(__codexBuildInfoTarget.packageFormat??__codexBuildInfoProfile.format,__codexBuildInfoText("unknown")),__codexBuildInfoText("enabledFeatures")+": "+(__codexBuildInfoFeatures.length>0?__codexBuildInfoFeatures.join(", "):__codexBuildInfoText("none")),__codexBuildInfoText("upstreamAppVersion")+": "+codexLinuxBuildInfoValue(__codexBuildInfoDmg.appVersion,__codexBuildInfoText("unknown")),__codexBuildInfoText("upstreamDmgSha256")+": "+codexLinuxBuildInfoValue(__codexBuildInfoDmg.sha256,__codexBuildInfoText("unknown")),__codexBuildInfoText("electron")+": "+codexLinuxBuildInfoValue(__codexBuildInfo.electronVersion,__codexBuildInfoText("unknown")),__codexBuildInfoText("linuxSourceCommit")+": "+__codexBuildInfoCommitValue,...(__codexBuildInfoCommitLink?[__codexBuildInfoText("sourceCommitUrl")+": "+__codexBuildInfoCommitLink]:[]),__codexBuildInfoText("sourceBranch")+": "+codexLinuxBuildInfoValue(__codexBuildInfoSource.branch,__codexBuildInfoText("unknown")),__codexBuildInfoText("generated")+": "+__codexBuildInfoGenerated].join("\\n")}async function codexLinuxOpenBuildInfoCommit(){let __codexBuildInfoResult=codexLinuxGetBuildInfo();return __codexBuildInfoResult.commitUrl?(await ${electronVar}.shell?.openExternal(__codexBuildInfoResult.commitUrl),{success:!0}):{success:!1}}async function codexLinuxShowBuildInfo(){try{let{t:__codexBuildInfoText}=codexLinuxBuildInfoI18n(),__codexBuildInfoResult=codexLinuxGetBuildInfo(),__codexBuildInfoCommitUrl=__codexBuildInfoResult.commitUrl,__codexBuildInfoPath=__codexBuildInfoResult.path,__codexBuildInfoButtons=[],__codexBuildInfoButtonIndex=0;__codexBuildInfoCommitUrl&&__codexBuildInfoButtons.push(__codexBuildInfoText("openSourceCommit")),__codexBuildInfoPath&&__codexBuildInfoButtons.push(__codexBuildInfoText("openMetadataFile")),__codexBuildInfoButtons.push(__codexBuildInfoText("ok"));let __codexBuildInfoBoxResponse=await ${electronVar}.dialog?.showMessageBox({type:"info",buttons:__codexBuildInfoButtons,defaultId:__codexBuildInfoButtons.length-1,cancelId:__codexBuildInfoButtons.length-1,message:__codexBuildInfoText("dialogTitle"),detail:codexLinuxBuildInfoDetail(__codexBuildInfoResult.info,__codexBuildInfoPath)});if(__codexBuildInfoCommitUrl&&__codexBuildInfoBoxResponse?.response===__codexBuildInfoButtonIndex++){await ${electronVar}.shell?.openExternal(__codexBuildInfoCommitUrl);return}if(__codexBuildInfoPath&&__codexBuildInfoBoxResponse?.response===__codexBuildInfoButtonIndex++)await ${electronVar}.shell?.openPath?.(__codexBuildInfoPath)}catch{}}`;
}
...
function applyLinuxBuildInfoTrayPatch(currentSource) {
  ...
  const buildInfoMenuLabel = 'codexLinuxBuildInfoI18n().t("buildInformation")';
  ...
  if (trayMenuMatch == null && !patchedSource.includes("role:`help`")) {
    ...
    trayMenuMatch != null &&
    !/getNativeTrayMenuItems\(\)\{[^]*?label:codexLinuxBuildInfoI18n\(\)\.t\("buildInformation"\),click:\(\)=>\{codexLinuxShowBuildInfo\(\)\}/.test(patchedSource)
  ) {
    const menuPrefix =
      `...process.platform===\`linux\`?[{label:${buildInfoMenuLabel},click:()=>{codexLinuxShowBuildInfo()}},{type:\`separator\`}]:[],`;
  ...
  if (
    !/\{role:`help`,id:[A-Za-z_$][\w$]*\.[A-Za-z_$][\w$]*\.help,submenu:\[\.\.\.process\.platform===`linux`\?\[\{label:codexLinuxBuildInfoI18n\(\)\.t\("buildInformation"\),click:\(\)=>\{codexLinuxShowBuildInfo\(\)\}\},\{type:`separator`\}\]:\[\],/.test(patchedSource)
  ) {
    if (canInstallHelper) {
      let patchedHelpMenu = false;
      patchedSource = patchedSource.replace(helpMenuRegex, (match) => {
        patchedHelpMenu = true;
        return `${match}...process.platform===\`linux\`?[{label:${buildInfoMenuLabel},click:()=>{codexLinuxShowBuildInfo()}},{type:\`separator\`}]:[],`;
      });
