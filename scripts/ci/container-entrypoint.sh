...
run_core() {
    ...
    if ! node --test scripts/patch-linux-window-ui.test.js scripts/patches/runner.test.js scripts/patches/core/quit-confirmation-focus/test.js scripts/lib/linux-features.test.js linux-features/*/test.js >"$node_test_log" 2>&1; then
