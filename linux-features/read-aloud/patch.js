...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function codexLinuxReadAloudSettingsOwner(){return\`${"${process.pid??0}"}-${"${Date.now()}"}-${"${Math.random().toString(16).slice(2)}"}\`}`,
    `async function codexLinuxReadAloudOpenRegularSettingsLock(codexLinuxReadAloudLockPath,codexLinuxReadAloudFlags){let codexLinuxReadAloudLockFile=await ${fsVar}.promises.open(codexLinuxReadAloudLockPath,codexLinuxReadAloudFlags|(${fsVar}.constants.O_NOFOLLOW??0));try{let codexLinuxReadAloudLockStat=await codexLinuxReadAloudLockFile.stat({bigint:!0});if(!codexLinuxReadAloudLockStat.isFile()){let codexLinuxReadAloudLockTypeError=Error(\`Settings lock is not a regular file: ${"${codexLinuxReadAloudLockPath}"}\`);throw codexLinuxReadAloudLockTypeError.code=\`EINVAL\`,codexLinuxReadAloudLockTypeError}return{file:codexLinuxReadAloudLockFile,stat:codexLinuxReadAloudLockStat}}catch(codexLinuxReadAloudLockOpenError){throw await codexLinuxReadAloudLockFile.close(),codexLinuxReadAloudLockOpenError}}`,
    `function codexLinuxReadAloudSameSettingsLock(codexLinuxReadAloudLeftStat,codexLinuxReadAloudRightStat){return codexLinuxReadAloudLeftStat.dev===codexLinuxReadAloudRightStat.dev&&codexLinuxReadAloudLeftStat.ino===codexLinuxReadAloudRightStat.ino}`,
    `async function codexLinuxReadAloudReleaseSettingsLock(codexLinuxReadAloudLock){let codexLinuxReadAloudOpenedLock=null;try{codexLinuxReadAloudOpenedLock=await codexLinuxReadAloudOpenRegularSettingsLock(codexLinuxReadAloudLock.path,${fsVar}.constants.O_RDONLY),(await codexLinuxReadAloudOpenedLock.file.readFile(\`utf8\`)).split(\`\\n\`,1)[0]===codexLinuxReadAloudLock.owner&&await ${fsVar}.promises.unlink(codexLinuxReadAloudLock.path)}catch{}finally{codexLinuxReadAloudOpenedLock!=null&&await codexLinuxReadAloudOpenedLock.file.close()}}`,
    `function codexLinuxReadAloudSettingsOwnerIsAlive(codexLinuxReadAloudOwner){let codexLinuxReadAloudOwnerPid=Number(codexLinuxReadAloudOwner.split(\`-\`,1)[0]);if(!Number.isInteger(codexLinuxReadAloudOwnerPid)||codexLinuxReadAloudOwnerPid<=0)return!1;try{return process.kill(codexLinuxReadAloudOwnerPid,0),!0}catch(codexLinuxReadAloudOwnerError){return codexLinuxReadAloudOwnerError?.code===\`EPERM\`}}`,
    `function codexLinuxReadAloudLiveRecoveryClaimOwner(codexLinuxReadAloudRecord,codexLinuxReadAloudNow){if(!codexLinuxReadAloudRecord.startsWith(\`recover:\`))return null;let codexLinuxReadAloudClaim=codexLinuxReadAloudRecord.slice(8),codexLinuxReadAloudSeparator=codexLinuxReadAloudClaim.lastIndexOf(\`:\`);if(codexLinuxReadAloudSeparator<0)return null;let codexLinuxReadAloudOwner=codexLinuxReadAloudClaim.slice(0,codexLinuxReadAloudSeparator),codexLinuxReadAloudExpires=Number(codexLinuxReadAloudClaim.slice(codexLinuxReadAloudSeparator+1));return Number.isFinite(codexLinuxReadAloudExpires)&&codexLinuxReadAloudExpires>codexLinuxReadAloudNow&&codexLinuxReadAloudSettingsOwnerIsAlive(codexLinuxReadAloudOwner)?codexLinuxReadAloudOwner:null}`,
    `async function codexLinuxReadAloudRecoverSettingsLock(codexLinuxReadAloudLockPath,codexLinuxReadAloudClaimOwner){let codexLinuxReadAloudRecoveryFlags=${fsVar}.constants.O_RDWR|(${fsVar}.constants.O_NOFOLLOW==null?0:${fsVar}.constants.O_APPEND),codexLinuxReadAloudOpenedLock=await codexLinuxReadAloudOpenRegularSettingsLock(codexLinuxReadAloudLockPath,codexLinuxReadAloudRecoveryFlags);try{let codexLinuxReadAloudLockSnapshot=await codexLinuxReadAloudOpenedLock.file.readFile(\`utf8\`),codexLinuxReadAloudOriginalOwner=codexLinuxReadAloudLockSnapshot.split(\`\\n\`,1)[0],codexLinuxReadAloudNow=Date.now();if(codexLinuxReadAloudNow-Number(codexLinuxReadAloudOpenedLock.stat.mtimeMs)<=3e4||codexLinuxReadAloudSettingsOwnerIsAlive(codexLinuxReadAloudOriginalOwner))return!1;let codexLinuxReadAloudClaim=\`recover:${"${codexLinuxReadAloudClaimOwner}"}:${"${codexLinuxReadAloudNow+1e3}"}\`;await codexLinuxReadAloudOpenedLock.file.write((codexLinuxReadAloudLockSnapshot.endsWith(\`\\n\`)?\`\`:\`\\n\`)+codexLinuxReadAloudClaim+\`\\n\`),await codexLinuxReadAloudOpenedLock.file.sync();let codexLinuxReadAloudCurrentLock=await codexLinuxReadAloudOpenRegularSettingsLock(codexLinuxReadAloudLockPath,${fsVar}.constants.O_RDONLY);try{if(!codexLinuxReadAloudSameSettingsLock(codexLinuxReadAloudOpenedLock.stat,codexLinuxReadAloudCurrentLock.stat))return!1;codexLinuxReadAloudLockSnapshot=await codexLinuxReadAloudCurrentLock.file.readFile(\`utf8\`)}finally{await codexLinuxReadAloudCurrentLock.file.close()}if(!codexLinuxReadAloudLockSnapshot.endsWith(\`\\n\`)||codexLinuxReadAloudLockSnapshot.split(\`\\n\`,1)[0]!==codexLinuxReadAloudOriginalOwner)return!1;let codexLinuxReadAloudFirstLiveClaim=codexLinuxReadAloudLockSnapshot.split(\`\\n\`).slice(1,-1).map(codexLinuxReadAloudRecord=>codexLinuxReadAloudLiveRecoveryClaimOwner(codexLinuxReadAloudRecord,codexLinuxReadAloudNow)).find(Boolean);if(codexLinuxReadAloudFirstLiveClaim!==codexLinuxReadAloudClaimOwner)return!1;let codexLinuxReadAloudTakeoverLock=await codexLinuxReadAloudOpenRegularSettingsLock(codexLinuxReadAloudLockPath,${fsVar}.constants.O_RDWR);try{if(!codexLinuxReadAloudSameSettingsLock(codexLinuxReadAloudOpenedLock.stat,codexLinuxReadAloudTakeoverLock.stat))return!1;let codexLinuxReadAloudTakeoverWrite=await codexLinuxReadAloudTakeoverLock.file.write(codexLinuxReadAloudClaimOwner+\`\\n\`,0,\`utf8\`);await codexLinuxReadAloudTakeoverLock.file.truncate(codexLinuxReadAloudTakeoverWrite.bytesWritten),await codexLinuxReadAloudTakeoverLock.file.sync()}finally{await codexLinuxReadAloudTakeoverLock.file.close()}let codexLinuxReadAloudVerifiedLock=await codexLinuxReadAloudOpenRegularSettingsLock(codexLinuxReadAloudLockPath,${fsVar}.constants.O_RDONLY);try{return codexLinuxReadAloudSameSettingsLock(codexLinuxReadAloudOpenedLock.stat,codexLinuxReadAloudVerifiedLock.stat)&&(await codexLinuxReadAloudVerifiedLock.file.readFile(\`utf8\`)).split(\`\\n\`,1)[0]===codexLinuxReadAloudClaimOwner}finally{await codexLinuxReadAloudVerifiedLock.file.close()}}finally{await codexLinuxReadAloudOpenedLock.file.close()}}`,
    `async function codexLinuxReadAloudAcquireSettingsLock(codexLinuxReadAloudSettingsPathValue){let codexLinuxReadAloudLockPath=\`${"${codexLinuxReadAloudSettingsPathValue}"}.lock\`,codexLinuxReadAloudLockOwner=codexLinuxReadAloudSettingsOwner(),codexLinuxReadAloudLockDeadline=Date.now()+2e3;for(;;)try{let codexLinuxReadAloudLockFile=await ${fsVar}.promises.open(codexLinuxReadAloudLockPath,\`wx\`,384),codexLinuxReadAloudLockInitError=null;try{await codexLinuxReadAloudLockFile.writeFile(codexLinuxReadAloudLockOwner+\`\\n\`),await codexLinuxReadAloudLockFile.sync()}catch(codexLinuxReadAloudLockWriteError){codexLinuxReadAloudLockInitError=codexLinuxReadAloudLockWriteError}finally{await codexLinuxReadAloudLockFile.close()}if(codexLinuxReadAloudLockInitError!=null){try{await ${fsVar}.promises.unlink(codexLinuxReadAloudLockPath)}catch{}throw codexLinuxReadAloudLockInitError}return{path:codexLinuxReadAloudLockPath,owner:codexLinuxReadAloudLockOwner}}catch(codexLinuxReadAloudLockError){if(codexLinuxReadAloudLockError?.code!==\`EEXIST\`)throw codexLinuxReadAloudLockError;try{if(await codexLinuxReadAloudRecoverSettingsLock(codexLinuxReadAloudLockPath,codexLinuxReadAloudLockOwner))return{path:codexLinuxReadAloudLockPath,owner:codexLinuxReadAloudLockOwner}}catch(codexLinuxReadAloudRecoveryError){if(codexLinuxReadAloudRecoveryError?.code!==\`ENOENT\`)throw codexLinuxReadAloudRecoveryError}if(Date.now()>=codexLinuxReadAloudLockDeadline)throw Error(\`Timed out waiting for settings lock ${"${codexLinuxReadAloudLockPath}"}\`);await new Promise(codexLinuxReadAloudRetry=>setTimeout(codexLinuxReadAloudRetry,25))}}`,
    `async function codexLinuxReadAloudWriteSettings(codexLinuxReadAloudSettingsPatch){let codexLinuxReadAloudSettingsPathValue=codexLinuxReadAloudSettingsPath();if(!codexLinuxReadAloudSettingsPathValue)throw Error(\`settings path unavailable\`);let codexLinuxReadAloudSettingsLock=null,codexLinuxReadAloudTempPath=null;try{await ${fsVar}.promises.mkdir(${pathVar}.dirname(codexLinuxReadAloudSettingsPathValue),{recursive:!0}),codexLinuxReadAloudSettingsLock=await codexLinuxReadAloudAcquireSettingsLock(codexLinuxReadAloudSettingsPathValue);let codexLinuxReadAloudSettingsObject=codexLinuxReadAloudSettings();Object.assign(codexLinuxReadAloudSettingsObject,codexLinuxReadAloudSettingsPatch),codexLinuxReadAloudTempPath=\`${"${codexLinuxReadAloudSettingsPathValue}"}.tmp.${"${codexLinuxReadAloudSettingsOwner()}"}\`;let codexLinuxReadAloudTempFile=await ${fsVar}.promises.open(codexLinuxReadAloudTempPath,\`wx\`,384);try{await codexLinuxReadAloudTempFile.writeFile(JSON.stringify(codexLinuxReadAloudSettingsObject,null,2)+\`\\n\`,\`utf8\`),await codexLinuxReadAloudTempFile.sync()}finally{await codexLinuxReadAloudTempFile.close()}await ${fsVar}.promises.rename(codexLinuxReadAloudTempPath,codexLinuxReadAloudSettingsPathValue),codexLinuxReadAloudTempPath=null;if(${fsVar}.constants.O_DIRECTORY!=null){let codexLinuxReadAloudSettingsDirectory=await ${fsVar}.promises.open(${pathVar}.dirname(codexLinuxReadAloudSettingsPathValue),${fsVar}.constants.O_RDONLY|${fsVar}.constants.O_DIRECTORY);try{await codexLinuxReadAloudSettingsDirectory.sync()}finally{await codexLinuxReadAloudSettingsDirectory.close()}}}finally{if(codexLinuxReadAloudTempPath!=null)try{await ${fsVar}.promises.unlink(codexLinuxReadAloudTempPath)}catch{}codexLinuxReadAloudSettingsLock!=null&&await codexLinuxReadAloudReleaseSettingsLock(codexLinuxReadAloudSettingsLock)}}`,
