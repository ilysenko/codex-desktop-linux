...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `async function codexLinuxReadAloudReleaseSettingsLock(codexLinuxReadAloudLock){try{(await ${fsVar}.promises.readFile(codexLinuxReadAloudLock.path,\`utf8\`)).split(\`\\n\`,1)[0]===codexLinuxReadAloudLock.owner&&await ${fsVar}.promises.unlink(codexLinuxReadAloudLock.path)}catch{}}`,
    ...
    `async function codexLinuxReadAloudRecoverSettingsLock(codexLinuxReadAloudLockPath,codexLinuxReadAloudClaimOwner){let codexLinuxReadAloudLockSnapshot=await ${fsVar}.promises.readFile(codexLinuxReadAloudLockPath,\`utf8\`),codexLinuxReadAloudOriginalOwner=codexLinuxReadAloudLockSnapshot.split(\`\\n\`,1)[0],codexLinuxReadAloudLockStat=await ${fsVar}.promises.stat(codexLinuxReadAloudLockPath);if(Date.now()-codexLinuxReadAloudLockStat.mtimeMs<=3e4||codexLinuxReadAloudSettingsOwnerIsAlive(codexLinuxReadAloudOriginalOwner))return;let codexLinuxReadAloudClaim=\`recover:${"${codexLinuxReadAloudClaimOwner}"}\`,codexLinuxReadAloudRecoveryRecords=codexLinuxReadAloudLockSnapshot.split(\`\\n\`).slice(1,-1);if(!codexLinuxReadAloudRecoveryRecords.includes(codexLinuxReadAloudClaim)){let codexLinuxReadAloudRecoveryFile=await ${fsVar}.promises.open(codexLinuxReadAloudLockPath,\`a\`);try{await codexLinuxReadAloudRecoveryFile.write((codexLinuxReadAloudLockSnapshot.endsWith(\`\\n\`)?\`\`:\`\\n\`)+codexLinuxReadAloudClaim+\`\\n\`),await codexLinuxReadAloudRecoveryFile.sync()}finally{await codexLinuxReadAloudRecoveryFile.close()}codexLinuxReadAloudLockSnapshot=await ${fsVar}.promises.readFile(codexLinuxReadAloudLockPath,\`utf8\`)}if(!codexLinuxReadAloudLockSnapshot.endsWith(\`\\n\`)||codexLinuxReadAloudLockSnapshot.split(\`\\n\`,1)[0]!==codexLinuxReadAloudOriginalOwner)return;let codexLinuxReadAloudFirstLiveClaim=codexLinuxReadAloudLockSnapshot.split(\`\\n\`).slice(1,-1).find(codexLinuxReadAloudRecord=>codexLinuxReadAloudRecord.startsWith(\`recover:\`)&&codexLinuxReadAloudSettingsOwnerIsAlive(codexLinuxReadAloudRecord.slice(8)));codexLinuxReadAloudFirstLiveClaim===codexLinuxReadAloudClaim&&await ${fsVar}.promises.readFile(codexLinuxReadAloudLockPath,\`utf8\`)===codexLinuxReadAloudLockSnapshot&&await ${fsVar}.promises.unlink(codexLinuxReadAloudLockPath)}`,
    `async function codexLinuxReadAloudAcquireSettingsLock(codexLinuxReadAloudSettingsPathValue){let codexLinuxReadAloudLockPath=\`${"${codexLinuxReadAloudSettingsPathValue}"}.lock\`,codexLinuxReadAloudLockOwner=codexLinuxReadAloudSettingsOwner(),codexLinuxReadAloudLockDeadline=Date.now()+2e3;for(;;)try{let codexLinuxReadAloudLockFile=await ${fsVar}.promises.open(codexLinuxReadAloudLockPath,\`wx\`,384),codexLinuxReadAloudLockInitError=null;try{await codexLinuxReadAloudLockFile.writeFile(codexLinuxReadAloudLockOwner+\`\\n\`),await codexLinuxReadAloudLockFile.sync()}catch(codexLinuxReadAloudLockWriteError){codexLinuxReadAloudLockInitError=codexLinuxReadAloudLockWriteError}finally{await codexLinuxReadAloudLockFile.close()}if(codexLinuxReadAloudLockInitError!=null){try{await ${fsVar}.promises.unlink(codexLinuxReadAloudLockPath)}catch{}throw codexLinuxReadAloudLockInitError}return{path:codexLinuxReadAloudLockPath,owner:codexLinuxReadAloudLockOwner}}catch(codexLinuxReadAloudLockError){if(codexLinuxReadAloudLockError?.code!==\`EEXIST\`)throw codexLinuxReadAloudLockError;try{await codexLinuxReadAloudRecoverSettingsLock(codexLinuxReadAloudLockPath,codexLinuxReadAloudLockOwner)}catch(codexLinuxReadAloudRecoveryError){if(codexLinuxReadAloudRecoveryError?.code!==\`ENOENT\`)throw codexLinuxReadAloudRecoveryError}if(Date.now()>=codexLinuxReadAloudLockDeadline)throw Error(\`Timed out waiting for settings lock ${"${codexLinuxReadAloudLockPath}"}\`);await new Promise(codexLinuxReadAloudRetry=>setTimeout(codexLinuxReadAloudRetry,25))}}`,
    `async function codexLinuxReadAloudWriteSettings(codexLinuxReadAloudSettingsPatch){let codexLinuxReadAloudSettingsPathValue=codexLinuxReadAloudSettingsPath();if(!codexLinuxReadAloudSettingsPathValue)throw Error(\`settings path unavailable\`);let codexLinuxReadAloudSettingsLock=null,codexLinuxReadAloudTempPath=null;try{await ${fsVar}.promises.mkdir(${pathVar}.dirname(codexLinuxReadAloudSettingsPathValue),{recursive:!0}),codexLinuxReadAloudSettingsLock=await codexLinuxReadAloudAcquireSettingsLock(codexLinuxReadAloudSettingsPathValue);let codexLinuxReadAloudSettingsObject=codexLinuxReadAloudSettings();Object.assign(codexLinuxReadAloudSettingsObject,codexLinuxReadAloudSettingsPatch),codexLinuxReadAloudTempPath=\`${"${codexLinuxReadAloudSettingsPathValue}"}.tmp.${"${codexLinuxReadAloudSettingsOwner()}"}\`;let codexLinuxReadAloudTempFile=await ${fsVar}.promises.open(codexLinuxReadAloudTempPath,\`wx\`,384);try{await codexLinuxReadAloudTempFile.writeFile(JSON.stringify(codexLinuxReadAloudSettingsObject,null,2)+\`\\n\`,\`utf8\`),await codexLinuxReadAloudTempFile.sync()}finally{await codexLinuxReadAloudTempFile.close()}await ${fsVar}.promises.rename(codexLinuxReadAloudTempPath,codexLinuxReadAloudSettingsPathValue),codexLinuxReadAloudTempPath=null}finally{if(codexLinuxReadAloudTempPath!=null)try{await ${fsVar}.promises.unlink(codexLinuxReadAloudTempPath)}catch{}codexLinuxReadAloudSettingsLock!=null&&await codexLinuxReadAloudReleaseSettingsLock(codexLinuxReadAloudSettingsLock)}}`,
