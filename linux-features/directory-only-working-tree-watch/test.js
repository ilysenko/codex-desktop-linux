...
function currentBundlePair(t, overrides = {}) {
  ...
  const sources = new Map([
    ["src-Bn_6ASpg.js", overrides.src ?? currentSrcSource()],
...
test("the worker patch injects one Watchbound adapter and is idempotent", () => {
  ...
  assert.match(legacy.reason, /current 26\.730\.61639 working-tree contract rejected/iu);
});
...
test("feature patch reports drift instead of patching an ambiguous bundle", () => {
  ...
  assert.match(result.reason, /current 26\.730\.61639 working-tree contract rejected/iu);
...
test("bundle discovery patches the current src and worker copies", (t) => {
  ...
  const targets = [
    path.join(buildDir, "src-Bn_6ASpg.js"),
...
test("patches the pristine 26.730.61639 bundle contract and accepts only its exact completed state", (t) => {
  ...
  const src = completed.get("src-Bn_6ASpg.js");
...
test("rejects markers outside the exact 26.730.61639 Watchbound handoff", (t) => {
...
test("rejects dual ownership, unrelated lookalikes, and unproven minified aliases", (t) => {
  const parcelCall =
    "rye(n,{ignoredPaths:[E.posix.join(n.path,`.git`)]})";
  const unmarkedHandoff = CURRENT_WATCHBOUND_ROUTE.replace(
    `/*${PARCEL_WATCH_MARKER}*/`,
    "",
  );
  ...
  const cases = [
    ...
    },
    {
      ...
      worker: currentWorkerSource(CURRENT_PARCEL_ROUTE.replace("rye(n,", "Qve(n,")),
    },
    {
    ...
    },
    {
      ...
      worker: currentWorkerSource().replace("new $ue(n,e)", "new $ue(other,e)"),
    },
  ];
...
test("rejects missing, duplicate, and partial current contracts", (t) => {
  ...
  const workerOriginalIndex = completedWorker.indexOf("var CurrentWorkerLocal");
  ...
  const workerBranchEnd = completedWorker.indexOf(";let t=sV()", workerBranchStart) + 1;
  ...
  const cases = [
    ...
    },
    {
      ...
      overrides: {
        worker: completedWorker.replace(
          `return ${HELPER_NAME}(this,e,`,
          `return missing${HELPER_NAME}(this,e,`,
        ),
        src: completedSrc,
      },
    },
...
test("restores both current bundles after injected first-write and second-write failures", (t) => {
...
test("rejects non-lossless UTF-8 bundles without changing their bytes", (t) => {
  const candidate = currentBundlePair(t);
  const srcPath = path.join(candidate.buildDir, "src-Bn_6ASpg.js");
  fs.appendFileSync(srcPath, Buffer.from([0xff]));
  const before = new Map(
    ["src-Bn_6ASpg.js", "worker.js"].map((name) => [
      name,
...
test("reports failed-integrity when rollback cannot prove original current bundle bytes", (t) => {
  ...
  assert.equal(
    fs.readFileSync(path.join(candidate.buildDir, "src-Bn_6ASpg.js"), "utf8"),
    before.get("src-Bn_6ASpg.js"),
  );
  assert.equal(
...
test("feature descriptors stage Watchbound before patching the worker", () => {
  assert.equal(WATCHBOUND_VERSION, "2.0.0");
  assert.deepEqual(
  ...
  assert.deepEqual(packageDescriptor.status({
    changed: true,
    runtime: { nodeRuntimeProbed: false },
    source: "inspect",
    version: WATCHBOUND_VERSION,
  }), {
    status: "skipped-optional",
    reason: `watchbound ${WATCHBOUND_VERSION} runtime qualification is unprobed`,
  });
});
...
test("the shipped artifact manifest pins the 2.0.0 source and four packages", () => {
...
test("Watchbound staging rejects unqualified target runtimes before materialization", async (t) => {
  ...
  await assert.rejects(
    ...
    /requires a probed target Node\.js version/u,
  );
  assert.equal(materializations, 0);
  ...
  await assert.rejects(
    ...
    /Target Electron version must be an exact semantic version/u,
  );
  assert.equal(materializations, 0);
...
test("a pinned Nix source build stages without registry-byte assumptions", async (t) => {
  const fixture = packageFixtureManifest(t);
...
test("Watchbound package staging selects ARM64 and rejects unsupported architectures", async (t) => {
  const fixture = packageFixtureManifest(t);
  const extractedDir = tempDirectory(t, "watchbound-arm64-app-");
  writeExtractedAppRuntime(extractedDir);
  const materializePackage = async (request) => ({
    packageDir: fixture.packages.get(request.name),
    integrity: request.integrity,
  ...
  assert.equal(fs.existsSync(packageTarget(
    extractedDir,
    "@gadicc/watchbound-node-linux-arm64-gnu",
  )), true);
  assert.equal(fs.existsSync(packageTarget(
  ...
  await assert.rejects(
    stageWatchboundPackages({
      extractedDir: tempDirectory(t, "watchbound-armv7-app-"),
      arch: "arm",
      manifest: fixture.manifest,
      materializePackage: async () => {
  ...
  try {
    await assert.rejects(
      ...
      (error) => {
        ...
        assert.match(error.message, /identity.*could not be proven/u);
        return true;
      },
    );
  } finally {
  ...
  await assert.rejects(
    stageWatchboundPackages(packageStageOptions(extractedDir, fixture, {
      ...
      renamePackage: (source, target) => {
        renames += 1;
        if (renames === 2) {
          // Keep the staged inode live so the replacement cannot recycle it
          // and accidentally satisfy the destination identity check.
          fs.renameSync(source, `${source}.displaced`);
          fs.mkdirSync(target, { recursive: true });
          markerPath = path.join(target, "concurrent-owner");
          fs.writeFileSync(markerPath, "preserve\n");
...
test("the adapter fails closed on every Watchbound 2.0.0 contract mismatch", async (t) => {
  t.after(() => {
  ...
  for (const [label, mutate] of mismatches) {
    ...
    await assert.rejects(
      ...
      /requires watchbound 2\.0\.0.*native exclusions/iu,
      label,
    );
...
test("the adapter rejects an unqualified root before creating a subscription", async (t) => {
  ...
  await assert.rejects(
    ...
    /root is not qualified \(filesystem-overlay\)/u,
  );
  assert.equal(fake.subscriptions.length, 0);
});
