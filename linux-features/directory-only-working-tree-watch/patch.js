...
} = require("../../scripts/patches/lib/transactional-files.js");
...
const WATCHBOUND_VERSION = "2.0.0";
...
function codexLinuxStartDirectoryOnlyWorkingTreeWatch(host, options, configuration) {
  return (async () => {
    ...
    const watchbound = globalThis[moduleOverrideKey] ?? await import("watchbound");
    if (
      watchbound.capabilities?.schemaVersion !== 8 ||
    ...
    const qualification = watchbound.qualifyRoot(lexicalRoot);
    if (qualification?.state !== "qualified") {
      const reasons = Array.isArray(qualification?.reasons)
        ? qualification.reasons.join(", ")
        : "unknown qualification result";
      throw new Error(
        `directory-only working-tree watch root is not qualified (${reasons})`,
      );
    }
...
function classifyCurrentBundle(bundlePath, source, settings) {
  const pristineLocalCount = countPattern(source, LOCAL_FILE_WATCH_METHOD);
  const completedLocalCount = countPattern(
    source,
  ...
  const rawRouteLookalikeCount = countPattern(source, PARCEL_WORKING_TREE_WATCH);
  ...
  const pristineRouteCount = pristineRouteMatches.length;
  const completedRouteCount = completedRouteMatches.length;
  const parcelInvocationCount = rawRouteLookalikeCount;
  const parcelHelperCount = parcelHelperMatches.length;
  ...
  const relevant =
    ...
    pristineRouteCount > 0 ||
    completedRouteCount > 0 ||
    parcelInvocationCount > 0 ||
    parcelHelperCount > 0 ||
    parcelImportCount > 0;
  return {
    branchCallCount,
...
function hasNoParcelRouteContract(record) {
  return record.markerCount === 0 &&
    ...
    record.parcelInvocationCount === 0 &&
...
function hasPristineWorkerRouteContract(record) {
  return record.pristineRouteCount === 1 &&
    ...
    record.parcelInvocationCount === 1 &&
...
const descriptors = [
  {
    ...
    status: (result) => ({
      status: result?.runtime?.nodeRuntimeProbed === false
        ? "skipped-optional"
        : result?.changed
          ? "applied"
      ...
      reason: result == null
        ...
        : result.runtime?.nodeRuntimeProbed === false
          ? `watchbound ${result.version} runtime qualification is unprobed`
