...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function codexLinuxWrapRestartIntentPath(){let __codexWrapStateDir=codexLinuxWrapAppStateDir();return __codexWrapStateDir?codexLinuxWrapPath().join(__codexWrapStateDir,\`codex-wrapper-updater\`,\`restart-intent\`):null}`,
    ...
    `function codexLinuxWrapWriteMarker(){let __codexWrapMarkerPath=codexLinuxWrapMarkerPath(),__codexWrapRestartIntentPath=codexLinuxWrapRestartIntentPath();if(!__codexWrapMarkerPath||!__codexWrapRestartIntentPath)return{ok:!1,reason:\`no-marker-path\`};let __codexWrapStatus=codexLinuxWrapReadStatus(),__codexWrapGeneration=typeof __codexWrapStatus?.candidate_wrapper_commit===\`string\`?__codexWrapStatus.candidate_wrapper_commit.trim():\`\`;if(!/^[0-9a-f]{40}$/.test(__codexWrapGeneration))return{ok:!1,reason:\`no-valid-candidate-generation\`};try{codexLinuxWrapFs().mkdirSync(codexLinuxWrapPath().dirname(__codexWrapMarkerPath),{recursive:!0});codexLinuxWrapFs().writeFileSync(__codexWrapRestartIntentPath,__codexWrapGeneration+\`\\n\`,{mode:384});codexLinuxWrapFs().writeFileSync(__codexWrapMarkerPath,__codexWrapGeneration+\`\\n\`,{mode:384});return{ok:!0,path:__codexWrapMarkerPath,generation:__codexWrapGeneration}}catch(e){try{codexLinuxWrapFs().unlinkSync(__codexWrapRestartIntentPath)}catch{}return{ok:!1,error:String(e?.message||e)}}}`,
...
function wrapperRuntimeSource() {
  return [
    ...
    `function ensureButton(){if(button&&document.contains(button))return button;installStyle();let b=document.createElement("button");b.type="button";b.className="codex-linux-wrapper-update-btn";b.setAttribute("aria-label","Update and reopen ChatGPT Desktop for Linux");b.title="Install the newer ChatGPT Desktop for Linux build and reopen once";b.innerHTML='<span class="cdx-wrap-glyph">\\u2193</span><span class="cdx-wrap-label">Update and reopen</span>';b.addEventListener("click",onClick);button=b;attachButton(b);return b}`,
    ...
    `async function onClick(){if(busy||button&&button.dataset.state==="dev-mode")return;busy=true;let b=ensureButton();b.disabled=true;setBtn(b,"\\u21bb","Reopening\\u2026");try{let r=await post({action:"install"});if(r&&r.body&&r.body.ok===false){setBtn(b,"\\u2193","Update and reopen");b.title=r.body.error||r.body.reason||"Update failed";setTimeout(()=>{b.title="Install the newer ChatGPT Desktop for Linux build and reopen once"},2400)}}catch{setBtn(b,"\\u2193","Update and reopen")}finally{busy=false;b.disabled=false}}`,
