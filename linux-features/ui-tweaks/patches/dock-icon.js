...
const currentPreviewGate = "function Zv(e){if(process.platform!==`darwin`)return null";
...
const patchedAppInfoResource =
  "function codexLinuxDockIconResourcePath(e){return process.platform===`linux`?(0,m.join)(process.resourcesPath,`dock-icon`,e):(0,m.join)(process.resourcesPath,e)}function Qv(e){if(e==null)return null;let t=l.app.isPackaged||process.platform===`linux`?codexLinuxDockIconResourcePath(e):null";
...
const patchedUpdateGate =
  "I=()=>{if(!g&&process.platform!==`linux`)return;let e=k();F(e),xme({preference:e,resourceName:e===`codex-system`?N.light:null}).then(e=>{e&&F(k())})}";
