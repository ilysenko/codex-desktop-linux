...
function buildPetOverlayMethods(settings) {
  return [
    ...
    "codexPetOverlaySelectNiriWindow(e,t){if(!Array.isArray(e))return null;let n=this.codexPetOverlayRect(t),r=[];for(let i of e){let e=Number(i?.id);if(!Number.isSafeInteger(e)||e<=0||String(i?.title??``)!==`Codex Pet Overlay`||Number(i?.pid)!==Number(process.pid))continue;if(i.is_floating!=null&&typeof i.is_floating!=`boolean`)continue;let t=this.codexPetOverlayNiriWindowSize(i),a=t==null?0:Math.abs(t.width-Number(n?.width??t.width))+Math.abs(t.height-Number(n?.height??t.height)),o=t==null?0:t.width*t.height;r.push({window:i,id:e,sizeScore:a,area:o})}if(n!=null){let e=r.filter(e=>e.sizeScore<=16);return e.length===1?e[0].window:null}let i=r.filter(e=>e.area>0&&e.area<=300000);return i.length===1?i[0].window:r.length===1?r[0].window:null}",
    ...
    "codexPetOverlayApplyNiriHints(e){if(process.platform!==`linux`||e==null||e.isDestroyed?.()||!this.codexPetOverlayShouldUseNiri())return;this.codexPetOverlayFindNiriWindow(e,t=>{if(e.isDestroyed?.()||this.window!==e)return;let n=Number(t?.id);if(!Number.isSafeInteger(n)||n<=0)return;t.is_floating!==!0&&this.codexPetOverlayNiri([`action`,`move-window-to-floating`,`--id`,String(n)]);let r=this.codexPetOverlayDesiredWindowBounds,i=Number(r?.x),a=Number(r?.y);[i,a].every(Number.isFinite)&&this.codexPetOverlayNiri([`action`,`move-floating-window`,`--id`,String(n),`-x`,String(Math.round(i)),`-y`,String(Math.round(a))])})}",
