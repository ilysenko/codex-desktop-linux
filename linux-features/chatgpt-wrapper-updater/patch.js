...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function codexLinuxWrapSettingsAppId(){let e=process.env.CODEX_LINUX_APP_ID||process.env.CODEX_APP_ID||\`chatgpt-desktop\`;return/^[A-Za-z0-9._-]+$/.test(e)?e:\`chatgpt-desktop\`}`,
...
function applyWrapperUpdateSettingsPatch(source) {
  let next = source
    .replaceAll("Codex Desktop Linux", "ChatGPT Desktop Linux")
    .replaceAll("codex-desktop-linux", "chatgpt-desktop-linux")
    .replaceAll("upstream Codex app", "upstream ChatGPT app")
    .replaceAll("when you close Codex", "when you close ChatGPT")
    .replaceAll("waits for Codex to close", "waits for ChatGPT to close");
  ...
  if (!next.includes("Check for ChatGPT Desktop Linux updates")) {
    const toggleNeedle =
    ...
    const existingWrapperToggle =
      `$.jsx(LinuxToggle,{settingKey:KEYS.wrapperUpdates,label:"Check for ChatGPT Desktop Linux updates",description:"Check for Linux wrapper updates from chatgpt-desktop-linux in addition to upstream ChatGPT app updates.",defaultValue:!1},"wrapperUpdates")`;
...
function applyWrapperUpdateGeneralSettingsPatch(source) {
  source = source
    ...
    .replaceAll("when you close Codex", "when you close ChatGPT")
    .replaceAll("waits for Codex to close", "waits for ChatGPT to close");
  const hasWrapperSetting = source.includes("Check for ChatGPT Desktop Linux updates");
